<?php
session_start();		
	  $user=$_SESSION['logged_user'];
	  if ($user=='')
		  header('location:http://127.0.0.1/Nist_forum/unreg.php');
	   else if(!isset($user))
		   header('location:http://127.0.0.1/Nist_forum/unreg.php');
    require_once('http://127.0.0.1/Nist_forum/header.php');
?>
<!------------------>
<?php
      error_reporting(E_ALL);
	function executequery($sql)
	{
		$con = mysqli_connect('localhost','root','');
	      mysqli_select_db($con,'dforum');
		$data=mysqli_query($con,$sql);
		mysqli_close($con);	
           return $data;		
	}
	function executenonquery($sql)
	{
		$con = mysqli_connect('localhost','root','');
	      mysqli_select_db($con,'dforum');
		$result=mysqli_query($con,$sql);
		mysqli_close($con);	
           return $result;		
	}		
?>
<head>

<style type='text/css'>
  #lufont
  {
	  font-family: "Arial";
	  font-style: bold italic;
	  font-size: 20px;
	  color: rgb(255,255,0);
  }
    #qfont
  {
	  font-family: "Times New Roman";
	  font-style: normal;
	  font-size: 20px;
	  color: rgb(0,0,255);
  }
  #ufont
  {
	  font-family: "Arial";
	  font-style: bold italic;
	  font-size: 20px;
	  color: rgb(255,0,0);
  }
</style>
<link rel="stylesheet" href="log.css" type="text/css" media="screen" />
</head>
 <table align='right'>
		<tr>
			<td align='right' id='lufont'> 
			<?php
			 //session_start();		
			 //echo $_SESSION['logged_user'];
			$uid=$_SESSION['logged_user'];
		
			if(($_SESSION['logged_user'])!='')
			{
				echo "User Login As:";
				$sq = "select user_name from user where user_id='".$_SESSION['logged_user']."'";
			    $dat=executequery($sq);
				
				if($re=mysqli_fetch_row($dat))
				{
					echo "<span style='color:skyblue;'>",$re[0],'</span>';
				}	
			}	
               else
					echo "<h1>Pls Login with mailid & password....</h1>";				   
			?>	
			</td>
		</tr>
		<tr>
			<td align='right'>
				<a href='logout.php'><span style='color:#99ff66;'><b>LOGOUT</b></span></a>
			</td>
		</tr>
	</table>
<!---------          -->
 <br><br><br>
 <form action='message_code.php' method='post'>
 <center>
<div class="b">
<table>       
	  <tr>
	      <td><font color='green'><b>TO:</b></font>
		  <select  name='user_name'>
			   <option value=''>&nbsp;</option>
			   <?php
				        $data=executequery('select user_name,user_id from user where user_id!='.$_SESSION['logged_user']);
					   while($rec=mysqli_fetch_row($data))
					  {
							echo "<option value='$rec[1]'>$rec[0]</option>"; 
					  }
			  ?>
		      </select></td>
	   </tr>   
	   <tr><td><font color='green'><b>ENTER YOUR MESSAGE:</b></font></td></tr>
	   <tr><td><textarea rows="3" cols="50" name='material'></textarea></td></tr>
	   <td align='center'><input class="button" type='submit' id="myBtn" value='Send' name='send'>
   </td>
</table>
</div>
 </center>
</form>
<!-----------------     ---->
<?php
  $data=executequery("select user_name,chat_material,date_time from chat,user where user_id=chat_sender_id and chat_receiver_id=".$_SESSION['logged_user']." order by date_time desc");
	 $i=0;
    while($rec=mysqli_fetch_row($data))
    {
     $i=$i+1;		
     $user_name=$rec[0];	
     $chat_material=$rec[1];
	$date_time=$rec[2];
	if($i%2==0)
	 {
	echo "<table border='5' bordercolor='maroon' align='center' height='10%' width='50%' >
	<tr bgcolor='white'>
	<td width='30%' id='ufont'>  <b>MESSAGE$i :</b><br>
	    by <b>$user_name</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;   $date_time	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
    </tr>
	</table>
	<table border='5' bordercolor='maroon' align='center' height='3%' width='50%'>
     <tr bgcolor='yellow'>
		<td id='qfont'>	$chat_material		
		<br>
         &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		</td>
	  </tr>
  </table>";
	 }
	  else
	 {
	echo "<table border='5' bordercolor='maroon' align='center' height='10%' width='50%' >
	<tr style='background-color:rgb(200,200,200);'>
	       <td width='30%' id='ufont'>  <b>MESSAGE$i :</b><br>
	    by <b>$user_name</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;   $date_time	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
    </tr>
	</table>
	<table border='5' bordercolor='maroon' align='center' height='3%' width='50%'>
     <tr style='background-color:rgb(153,255,153);'>
		<td id='qfont'>	$chat_material 
		<br>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		</td>
	</tr>	 
	 </table>";
	 }	
     echo "<br>";	 
 }
?>  

<?php
    require_once('http://127.0.0.1/Nist_forum/footer.php');
?>